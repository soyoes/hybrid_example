...
typedef void(^http_f)(obj_t, dic_t, int res_code);
...
@property (nonatomic,retain) str_t placeHolder;
...
typedef void(^EditHandler)($&, bool);
typedef void(^ObserveValueForKeyPathHandler)(str_t keyPath, obj_t object, dic_t change, void * context);
typedef void(^LayoutSubviewsHandler)();

typedef void(^ScrollWillBeginDraggingHandler)(UIScrollView *scrollView);
typedef void(^ScrollWillEndDraggingHandler)(UIScrollView *scrollView,CGPoint velocity, CGPoint * targetContentOffset);
typedef void(^ScrollEndDraggingHandler)(UIScrollView *scrollView, BOOL decelerate);
typedef void(^ScrollDidScroll)(UIScrollView *scrollView);
typedef void(^ScrollToTopHandler)(UIScrollView *scrollView);
typedef void(^ScrollWillBeginDeceleratingHandler)(UIScrollView *scrollView);
typedef void(^ScrollDidEndDeceleratingHandler)(UIScrollView *scrollView);

typedef void(^TextViewDidChangeHandler)(UITextView *textView);

#pragma mark - View
@interface LBView : UIScrollView<UITextViewDelegate,UIPickerViewDelegate,UIGestureRecognizerDelegate>
@property (nonatomic,retain) mdic_t     gestures;
@property (nonatomic,retain) mdic_t     gesmap;
@property (nonatomic,retain) mdic_t     animates;
@property (nonatomic,retain) ges_t      ges_a;
@property (nonatomic,retain) mdic_t     data;
@property (nonatomic,retain) textview_t textField;
@property (nonatomic,retain) text_layer_t    placeHolderLayer;    //3rd sublayer of contentLayer, text
...
@property (nonatomic,retain) str_t placeHolder, placeHolderColor;
...
@property (nonatomic,copy) ObserveValueForKeyPathHandler observer_handler;
@property (nonatomic,copy) LayoutSubviewsHandler layoutSubviewsHandler;
@property (nonatomic,copy) ScrollWillBeginDraggingHandler       scrollWillBeginDraggingHandler;
@property (nonatomic,copy) ScrollWillEndDraggingHandler         scrollWillEndDraggingHandler;
@property (nonatomic,copy) ScrollEndDraggingHandler             scrollEndDraggingHandler;
@property (nonatomic,copy) ScrollDidScroll                      scrollHandler;
@property (nonatomic,copy) ScrollToTopHandler                   scrollToTopHandler;
@property (nonatomic,copy) ScrollWillBeginDeceleratingHandler   scrollWillBeginDeceleratingHandler;
@property (nonatomic,copy) ScrollDidEndDeceleratingHandler      scrollDidEndDeceleratingHandler;
@property (nonatomic,copy) TextViewDidChangeHandler             textViewDidChangeHandler;
...
#pragma mark - LB Delegate implements
@interface LBGesture :NSObject <UIGestureRecognizerDelegate>
@property (nonatomic,assign) bool shouldRecognizeSimultaneously;
...
typedef void(^HTTPRequestHandler)(obj_t, dic_t, int res_code);
...
@property (nonatomic)           NSInteger           res_statusCode;
@property (nonatomic, copy)     HTTPRequestHandler  handler;
@property (nonatomic, retain)   str_t    url;
@property (nonatomic, retain)   str_t    method;

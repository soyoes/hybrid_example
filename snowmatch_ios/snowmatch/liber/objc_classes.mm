...
bool __has_firstResponder = false;
int __firstResponder_idx = -1;
...
str_t style_t::encode(style_t s){
    ...
    [arr addStringKeyValue:@"placeHolderColor"  value:s.placeHolderColor  separator:delimiter_k_v];
...
style_t style_t::decode(str_t str) {
    if (str) {
        ...
        for (NSString* data in arr) {
            ...
            else if (str_eq(@"placeHolderColor",key)) ss.placeHolderColor = isNull ? ss.placeHolderColor : value;
...
@implementation LBGesture
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.shouldRecognizeSimultaneously = false;
    }
    return self;
}
...
-(id) initWithOwner:($*)owner rect:(rect_t)rect viewid:(str_t)ID idx:(int)idx{
    ...
    fillmode = kFill; // image default fillmode
    
    _shouldRecognizeSimultaneously = false;
    return self;
}

-(void) addObserver:(str_t)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context ohandler:(ObserveValueForKeyPathHandler)ohandler lhandler:(LayoutSubviewsHandler)lhandler {
    _observer_handler = ohandler;
    _layoutSubviewsHandler = lhandler;
    [self addObserver:self forKeyPath:keyPath options:options context:context];
}
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    [self setNeedsLayout];
    if (_observer_handler) {
        _observer_handler(keyPath, object, change, context);
    }
}
-(void)layoutSubviews {
    [super layoutSubviews];
    if (_layoutSubviewsHandler) {
...
-(void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if(_scrollWillBeginDeceleratingHandler) _scrollWillBeginDeceleratingHandler(scrollView);
}
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if(_scrollDidEndDeceleratingHandler) _scrollDidEndDeceleratingHandler(scrollView);
}
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if(_scrollWillBeginDraggingHandler) _scrollWillBeginDraggingHandler(scrollView);
}
-(void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if(_scrollWillEndDraggingHandler) _scrollWillEndDraggingHandler(scrollView,velocity, targetContentOffset);

}
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if(_scrollEndDraggingHandler) _scrollEndDraggingHandler(scrollView, decelerate);
}
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(_scrollHandler) _scrollHandler(scrollView);
}
-(void)scrollViewDidScrollToTop:(UIScrollView *)scrollView {
    if(_scrollToTopHandler) _scrollToTopHandler(scrollView);
}
...
-(void) switchEditingMode{
    ...
    if(_textField!=nil){
        ...
        if(_textField.hidden){
            ...
            // scrollTop
            __has_firstResponder = true;
            __firstResponder_idx = [_owner->view() idx];
            if (firstResponder) {
                if ([[firstResponder superview] isKindOfClass:[LBView class]]) {
                    LBView * parent = (LBView*)[firstResponder superview];
                    str_t new_text = ((textview_t)(parent.textField)).text;
                    parent.owner->text(new_text);
                    
                    parent.textField.hidden = YES;
                }
            } else {
                CGRect rWindow = [ _textField convertRect:_textField.frame toView:nil];
                CGRect rScreen = [ _textField.window convertRect:rWindow toView:nil];
                float top = 20-(rScreen.origin.y - rScreen.size.height);
                top = top > 0 ? 0: top;
                _owner->scrollTop(top);
            }
            ...
            __has_firstResponder = false;
            __firstResponder_idx = -1;
        }
    }
}
...
-(void) setStyles:(style_t)ss{
    ...
    if(need_redraw){
        ...
        if(_imageLayer)
            _imageLayer.frame = layer.bounds;
        if(_textLayer)
    ...
    if (ss.align)   textalign= (align_t)[TEXTALIGNS indexOfObject:ss.align];
    if(textalign<0) textalign = kTopLeft;
    color    = ss.color?str2rgba(ss.color):(rgba_t){0,0,0,1};
    
    //float p = numf(ss.padding);
    padding.x = numf(ss.paddingLeft, ss.padding, padding.x);
    padding.y = numf(ss.paddingTop, ss.padding, padding.y);
    padding.z = numf(ss.paddingRight, ss.padding, padding.z);
    padding.w = numf(ss.paddingBottom, ss.padding, padding.w);
//    padding = {
...
//    };
    ...
    if (ss.placeHolder) placeHolder = ss.placeHolder;
    if (ss.placeHolderColor) placeHolderColor = ss.placeHolderColor;
...
-(void) _setShadow:(str_t)shadow{
    [self _clearShadows];
    if(!shadow || str_eq(shadow, @"none"))return;
...
-(void) setImage:(id)src{
    ...
    if([src isKindOfClass:[NSString class]]){
        ...
        if([src hasPrefix:@"http:"]||[src hasPrefix:@"https:"]||[src hasPrefix:@"ftp:"]){//URL
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
                ...
                if (url) {
                    ...
                    NSLog(@"Incorrect url format:%@", _src);
                }
        ...
        }else if([src hasPrefix:@"/"]){// absolute path
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
                @autoreleasepool {
...
-(void) setText:(str_t)txt{
    ...
    if (placeHolder && ![@"" isEqualToString:placeHolder]) {
        if (_placeHolderLayer == nil) {
            _placeHolderLayer= [[CATextLayer alloc] init];
            [_contentLayer addSublayer:_placeHolderLayer];
            
            if([_placeHolderLayer respondsToSelector:@selector(setContentsScale:)])
                _placeHolderLayer.contentsScale = [[UIScreen mainScreen] scale];
            _placeHolderLayer.hidden = NO;
        }
        
        if (_text == nil || _text.length == 0) {
            [_placeHolderLayer setString:placeHolder];
        } else {
            [_placeHolderLayer setString:@""];
        }
    }
...
-(void) _applyTextstyle{
    ...
    if (_placeHolderLayer) {
        [_placeHolderLayer setFrame:_textLayer.frame];
        // TODO: color
        
        color_t cl = placeHolderColor ? rgba2color(str2rgba(placeHolderColor)) : rgba2color(str2rgba(@"#AFB6BA"));
        [_placeHolderLayer setForegroundColor:cl.CGColor];
        
        [_placeHolderLayer setBackgroundColor: [UIColor clearColor].CGColor];
        [_placeHolderLayer setFont:_textLayer.font];
        [_placeHolderLayer setFontSize:_textLayer.fontSize];
        _placeHolderLayer.wrapped = _textLayer.wrapped;
        _placeHolderLayer.truncationMode = _textLayer.truncationMode;
        [_placeHolderLayer setAlignmentMode:_textLayer.alignmentMode];
        _placeHolderLayer.zPosition = _textLayer.zPosition +1;
    }
}
-(void)setEditable:(BOOL)ea{[self setEditable:ea handler:^($&v,bool isInit){}];}
-(void)setEditable:(BOOL)ea handler:(EditHandler)handler{
    if(_textField==nil){
...
-(void) setPickable:(arr_t) opts{
    [self setPickable:opts handler:^($&v,bool isInit){}];
}
-(void) setPickable:(arr_t) opts handler:(EditHandler)handler{
    if(!_text){
        NSLog(@"ERR: setPickable requires label()");
        return;
    }
    [self setEditable:true handler:handler];
...
-(void) setPickable:(date_t) date format:(str_t) format{
    [self setPickable:date format:format handler:^($&v,bool isInit){}];
}
-(void) setPickable:(date_t) date format:(str_t) format handler:(EditHandler)handler{
    if(!_text){
        NSLog(@"ERR: setPickable requires label()");
        return;
    }
...
-(void) bind:(str_t)event handler:(ges_f)handler opts:(dic_t)opts {
    ...
    if([[event lowercaseString] isEqualToString:@"swipe"]){
...
-(void)unbind:(str_t) event{
    ...
    if(gh){
        ...
        if(_gesmap[event]) {
            [self removeGestureRecognizer:_gesmap[event]];
            mdic_remove(_gesmap, event);
        }
    }
}
...
-(void) animate:(float)ms step:(anime_step_f)onStep end:(anime_end_f)onEnd opts:(anime_t)opts{
    ...
    set_interval(interval, ^bool(dic_t d, int i){
        ...
        if(d[@"onStep"]){
            ...
            o.owner->set(@"__animate_tag", @(opts.tag));
...
-(view_t) root{
    ...
    while (p) {
        // check parent
        view_t parent = ((view_t)p).parent;
        if (!is_view(parent)) break;
//        // check controller
//        obj_t is_con = [parent get:@"__is_controller__"];
//        NSNumber* is_controller = is_con == nil ? @NO : (NSNumber*)is_con;
//        if ([is_controller  isEqual: @YES] ) break;
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([text isEqualToString:@"\n"]) {
        if(!((textview_t)_textField).nowrap){
            ...
            _owner->scrollBack();
            EditHandler eh = _owner->get(@"editHandler");
            if(eh) eh(*_owner,false);
            __has_firstResponder = false;
            __firstResponder_idx = -1;
            return NO;
        }
...
-(void)textViewDidChange:(UITextView *)textView {
    if(_textViewDidChangeHandler) _textViewDidChangeHandler(textView);
...
- (UIView *)inputAccessoryView {
    ...
    auto fh =^(ges_t g, $& n, dic_t p) {
        ...
        if(owner){
            ...
            owner->scrollBack();
            EditHandler eh = owner->get(@"editHandler");
            if(eh) eh(*owner,false);
            __has_firstResponder = false;
            __firstResponder_idx = -1;
        }
    };
    //$&b =box({@0, @0, @(SW), @(SH), @0, @"#ffffff11"}).bind(@"tap",fh, @{});
    if (!inputAccessoryView && !picker && (datePicker || !nowrap)) {
        $&b =box({@0, @0, @(SW), @44, @0, @"#ECF0F1"})
...
+ (NSString *) makeQueryStr:(NSDictionary *)param{
    ...
    for (NSString *key in param) {
        ...
        NSString *value;
        if([v isKindOfClass:[NSDictionary class]] || [v isKindOfClass:[NSArray class]] || [v isKindOfClass:[NSData class]] ||[v isKindOfClass:[UIImage class]]) {
            value = [HTTPRequest pack:(NSArray *)v];
        } else {// url encode
            str_t str = sprintf(@"%@", v);
            str_t res = [str stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet alphanumericCharacterSet]];
            value = res;
        }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSHTTPURLResponse *)response {
    ...
    self.res_statusCode = [response statusCode];
    if (!([response statusCode] >= 200 && [response statusCode] < 300 && [response statusCode] != 204)) {
        NSLog(@"Failed to get Data. Response status code: %d. url:%@\n method:%@\n Params: \n%@\n\n", (int)[response statusCode], self.url, self.method, self.args ? self.args : @{});
    }
}
...
- (void) connectionDidFinishLoading: (NSURLConnection*) connection {
    ...
    NSError * err = nil;
    id res = self.raw.length? [NSJSONSerialization JSONObjectWithData:self.raw options:NSJSONReadingMutableContainers error:&err]:nil;
    if (err) {
        NSLog(@"\nError: Parse response error!\n====== Response status code: %@\n====== Error code: %@\n====== Error localizedDescription: %@\n ======url: %@\n ======method: %@\n======Params: \n%@\n ====== Response data:\n%@\n\n" ,
              ...
              self.method,
              self.args ? self.args : @{},
              [[NSString alloc] initWithData:self.raw encoding:NSASCIIStringEncoding]);
        res = @{@"error": @(err.code)}; //Parse data error
    }
    self.handler(res, self.args, (int)self.res_statusCode);
}

-(void) connection:(NSURLConnection *)connection didFailWithError: (NSError *)error {
    //[activityIndicator stopAnimating];NSURLErrorDomain
    NSLog (@"Connection Failed with Error. %@", error.description);
    self.handler(@{@"error": @(error.code), @"conn_error": error}, self.args, (int)self.res_statusCode); // Connection error.
}

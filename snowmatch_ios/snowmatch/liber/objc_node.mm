...
$& $::shouldRecognizeSimultaneously(bool should) {
    view_t v = view();
    if(v) v.shouldRecognizeSimultaneously = should;
    return *this;
}
...
void $::remove(){
    view_t v = view();
    if(v){
//        view_t o_v = v.owner->view();
//        if (o_v && o_v.nodes && [o_v.nodes count] > 0) {
//            int idx =  [o_v.nodes indexOfObject:v];
//            marr_remove(o_v.nodes, idx);
//        }
//
        if (v.pages) [v.pages removeFromSuperview];
        [v removeFromSuperview];
        if(__nodes && __nodes[v.NS]){
            [__nodes[v.NS] removeObjectForKey:v.ID];
        }
        if(__idxmap && __idxmap[@(view_id)])
            [ __idxmap removeObjectForKey:@(view_id) ];
    }
    //delete this;
}
void $::remove_children() {
    view_t v = view();
    if (v && v.nodes && [v.nodes count] > 0) {

        while ([v.nodes lastObject]) {
            view_t last_v = [v.nodes lastObject];
            [v.nodes removeLastObject];
            last_v.owner->remove();
        }
    }
}
...
long $::getLong(str_t key){obj_t r = get(key); return r? numl(r):0;}
...
$& $::scrollBack(){
    ...
    if(v){
        ...
        if(p){
            ...
            if(off){
                [p setContentOffset:{0,[off floatValue]} animated:YES];
...
__attribute__((overloadable)) $& $::setPickable(arr_t opts){
    return setPickable(opts,  ^($&v, bool isInit){});
}
__attribute__((overloadable)) $& $::setPickable(arr_t opts, EditHandler handler){
    view_t v = view();
    if(v) [v setPickable:opts handler:handler];
    return *this;
}
__attribute__((overloadable)) $& $::setPickable(date_t date, str_t format){
    return setPickable(date, format,  ^($&v, bool isInit){});
}
__attribute__((overloadable)) $& $::setPickable(date_t date, str_t format, EditHandler handler){
    view_t v = view();
    if(v) [v setPickable:date format:format handler:handler];
    return *this;
}

__attribute__((overloadable)) $& $::setEditable(BOOL editable){return setEditable(editable, ^($&v,bool isInit){});}
__attribute__((overloadable)) $& $::setEditable(BOOL editable, EditHandler handler){
    view_t v = view();
    if(v) [v setEditable:editable handler:handler];
    return *this;
}
